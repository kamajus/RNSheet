...
```ts
import {SheetDefinition} from 'rn-sheet';
...
See [Scrolling guide](./scrolling.mdx) to learn how to migrate away from `useScrollHandlers` and use dedicated scrolling views provided by `rn-sheet`.
...
```tsx
import { ActionSheetRef } from 'rn-sheet';
const ExampleSheet = () => {
